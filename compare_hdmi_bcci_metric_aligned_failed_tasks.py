...
DEFAULT_TRUST_BETA_GRID = [0.05, 0.1, 0.2]
DEFAULT_SEL_WEIGHT_GRID = [1.0, 2.0]
...
def metric_aligned_bcci_intervention(
    model,
    h: torch.Tensor,
    target_token_id: int,
    ...
    for _ in range(max(1, int(inner_steps))):
        with torch.enable_grad():
            ...
            comp_loss = tv_distance_torch(p_c_after, goal)
            ...
            if vZe is not None and p_ze_before is not None:
                ...
                if m > 0:
                    sel_loss = tv_distance_torch(p_ze_after, p_ze_before) / m
...
def evaluate_metric_aligned_bcci(
    model,
    tokenizer,
    samples,
    vZc,
    ...
    for sample in samples:
        ...
        h1 = metric_aligned_bcci_intervention(
            model=model,
            h=h0,
...
def run_task(
    ...
    sel_weight_grid: Sequence[float],
    ...
    for gamma in gamma_grid:
        for trust_beta in trust_beta_grid:
            for sel_weight in sel_weight_grid:
                for anchor_weight in anchor_weight_grid:
                    metrics = evaluate_metric_aligned_bcci(
                        model=model,
                        tokenizer=tokenizer,
                        samples=tune_samples,
                        vZc=vZc,
    ...
    if best_cfg is not None:
        improved_test = evaluate_metric_aligned_bcci(
            model=model,
            tokenizer=tokenizer,
            samples=test_samples,
            vZc=vZc,
            ...
            sel_weight=float(best_cfg["sel_weight"]),
            anchor_weight=float(best_cfg["anchor_weight"]),
        )
    ...
    return {
        ...
        "search_grid": {
            ...
            "sel_weight": list(sel_weight_grid),
            "anchor_weight": list(anchor_weight_grid),
        },
        "hdmi_tune": {
        ...
        "selected_metric_aligned_config": best_cfg,
        "selected_metric_aligned_tune_metrics": best_metrics,
        "metric_aligned_test": improved_test,
    }
...
def main():
    ...
    parser.add_argument("--sel_weight_grid", nargs="+", type=float, default=DEFAULT_SEL_WEIGHT_GRID)
    parser.add_argument("--anchor_weight_grid", nargs="+", type=float, default=DEFAULT_ANCHOR_WEIGHT_GRID)
    parser.add_argument("--output", default="compare_hdmi_bcci_metric_aligned_failed_tasks.json")
    ...
    results = {
        "method": "BCCI-metric-aligned-hinge",
        "tasks": [],
    ...
    for task in args.tasks:
        print(f"Running metric-aligned BCCI on {task}")
        task_result = run_task(
            ...
            sel_weight_grid=args.sel_weight_grid,
