...
DEFAULT_PHASE1_FRAC_GRID = [0.5, 0.75]
DEFAULT_COMP1_WEIGHT_GRID = [2.0, 4.0]
DEFAULT_SEL1_WEIGHT_GRID = [0.0, 0.25]
DEFAULT_TRUST1_GRID = [0.0, 0.02]
DEFAULT_TRUST2_GRID = [0.05, 0.1]
...
dpas = load_module(DPAS_PATH, "dpas_curr_mod")
cmp = dpas.cmp
hdmi = dpas.hdmi
...
def curriculum_intervention(
    *,
    ...
    phase1_steps = int(round(max(1, int(inner_steps)) * float(phase1_frac)))
    phase1_steps = min(max(1, phase1_steps), max(1, int(inner_steps)))

    for step in range(max(1, int(inner_steps))):
    ...
    *,
    ...
    for sample in samples:
        ...
        h1 = curriculum_intervention(
            ...
            sel1_weight=sel1_weight,
            trust_beta1=trust_beta1,
...
def run_task(
    *,
    ...
    sel1_weight_grid: Sequence[float],
    trust_beta1_grid: Sequence[float],
    ...
    for phase1_frac in phase1_frac_grid:
        for comp1_weight in comp1_weight_grid:
            for sel1_weight in sel1_weight_grid:
                for trust_beta1 in trust_beta1_grid:
                    for trust_beta2 in trust_beta2_grid:
                        metrics = evaluate_curriculum(
                            model=model,
                            tokenizer=tokenizer,
                            samples=tune_samples,
                            vZc=vZc,
                            ...
                            sel1_weight=float(sel1_weight),
                            trust_beta1=float(trust_beta1),
    ...
    test_metrics = evaluate_curriculum(
        model=model,
        tokenizer=tokenizer,
        ...
        sel1_weight=float(selected["config"]["sel1_weight"]),
        trust_beta1=float(selected["config"]["trust_beta1"]),
    ...
    return {
        ...
        "selection_protocol": {
            "description": "maximize completeness subject first to after_task_acc <= HDMI_tune.after and selectivity >= HDMI_tune.selectivity; fallback to unconditional completeness-max curriculum",
            "hdmi_tune": hdmi_tune,
            "feasible_found": best_targeted_feasible is not None,
        },
        "selected_curriculum": selected,
        "curriculum_test": test_metrics,
    }
...
def main():
    ...
    parser.add_argument("--sel1_weight_grid", nargs="+", type=float, default=DEFAULT_SEL1_WEIGHT_GRID)
    parser.add_argument("--trust_beta1_grid", nargs="+", type=float, default=DEFAULT_TRUST1_GRID)
    parser.add_argument("--trust_beta2_grid", nargs="+", type=float, default=DEFAULT_TRUST2_GRID)
    parser.add_argument("--output", default="compare_hdmi_dpas_curriculum_c_only.json")
    ...
    results = {
        "method": "DPAS-curriculum-completeness-first",
        "tasks": [],
    ...
    for task in args.tasks:
        print(f"Running curriculum DPAS on {task}")
        task_result = run_task(
            ...
            sel1_weight_grid=args.sel1_weight_grid,
            trust_beta1_grid=args.trust_beta1_grid,
            trust_beta2_grid=args.trust_beta2_grid,
            dual_results_map=dual_results_map,
        )
        results["tasks"].append(task_result)
        Path(args.output).write_text(json.dumps(results, indent=2), encoding="utf-8")
        print(json.dumps(task_result["selected_curriculum"], indent=2))
